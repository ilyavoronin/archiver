...
class HuffmanTree {
  public:
    explicit HuffmanTree(const String <mchar> &data);
    HuffmanTree();
    ...
    std::map <mchar, String<bool> > getCodes();
    ...
    mchar getNextmchar(String <bool> &data, int &pos);

  private:
    struct Node {
        Node(mchar symb_ = 0, Node* left_ = nullptr, Node* right_ = nullptr);
    ...
    Node* buildTree(const String <mchar> &data);

    //`cur_str` is a bit string from root of the tree to the `cur_vert`
    //res[s] = bit string that corresponds to s in the tree
    void getCodesDfs(Node *cur_vert, String<bool> cur_str, std::map <mchar, String<bool> > &res);
    ...
    void getEulerDfs(Node *cur_vert, String <mchar> &symb, String <bool> &euler);
